...
handleOk(): void {
  ...
  if (this.modalType === 'created') {
    this.http.requestSaveCategory(title, content).subscribe(() => {
      this.onResetForm()
    ...
    this.http.requestUpdateCategory(this.rowId, title, content).subscribe(() => {
      this.onResetForm()
      this.isVisible = false;
      this.tableDataInit();
...
onDeleteRow(id: string): void {
  this.http.requestDeleteCategory(id).subscribe(() => {
    console.log('删除成功');
